...
async function build(){
 ...
 for(const name of ['kastor-logo.png','sora-medium.ttf','sora-bold.ttf'])write('brand/'+name,fs.readFileSync(path.join(root,'assets/brand',name)));
 for(const name of ['site.css','site.js'])write(name,fs.readFileSync(path.join(root,'source',name)));
 ...
 const cards=products.map(p=>`<a class="card" ${cardAttrs(p)} href="${route(p)}"><div class="photo"><img src="${picture(p)}" alt="${esc(p.name)}" loading="lazy"></div><p class="eyebrow">${esc(p.series)} SERIES ${p.demo?' · DEMO':''}</p><h2>${esc(p.name)}</h2><span class="small">Lihat spesifikasi ↗</span></a>`).join('');
 write('index.html',shell('Product Collection',`<section class="hero"><p class="eyebrow">KASTOR LIGHTING / COLLECTION</p><h1>Light for<br><em>Every Space.</em></h1><p>Temukan pencahayaan untuk setiap sudut ruang.</p><a href="#collection" class="button">Jelajahi koleksi ↓</a><span class="hero-circle" aria-hidden="true"></span></section><section id="collection"><div class="section-top"><h2>Our collection</h2><span class="small">1 produk · 10 contoh demo</span></div><p class="notice">Produk berlabel DEMO menggunakan foto AI dan spesifikasi dummy.</p>${filters()}<div class="grid">${cards}</div></section>`));
