...
function filters(){return `<section class="filters" aria-label="Cari produk"><label>Cari produk<input id="search" type="search" placeholder="Nama atau jenis lampu…"></label><label>Seri<select id="series"><option value="">Semua seri</option>${[...new Set(products.map(p=>p.series))].map(s=>`<option>${esc(s)}</option>`).join('')}</select></label></section><section id="subcategory-menu" aria-label="Submenu Iris" hidden><p class="eyebrow">IRIS → JENIS PRODUK</p><div class="series-chips"><button type="button" data-subcategory="" aria-pressed="true">Semua produk Iris</button><button type="button" data-subcategory="Retrofit Downlight" aria-pressed="false">Retrofit Downlight</button></div></section><section id="type-menu" aria-label="Submenu Retrofit Downlight" hidden><p class="eyebrow">IRIS → RETROFIT DOWNLIGHT → TIPE</p><div class="series-chips"><button type="button" data-type="" aria-pressed="true">Semua tipe</button><button type="button" data-type="Pinhole" aria-pressed="false">Pinhole</button></div></section><p id="count" class="small" aria-live="polite">${products.length} produk</p><p id="empty" hidden>Tidak ada produk yang cocok. Coba kata lain atau reset filter.</p>`;}
function cardAttrs(p){return `data-product data-search="${esc((p.name+' '+p.specs.Type).toLowerCase())}" data-series="${esc(p.series)}" data-subcategory="${esc(p.subcategory||'')}" data-type="${esc(p.specs.Type||'')}"`;}
async function qr(p){
...
async function build(){
 ...
 write('index.html',shell('Product Collection',`<section class="hero"><p class="eyebrow">KASTOR LIGHTING / COLLECTION</p><h1>Light for<br><em>Every Space.</em></h1><p>Temukan pencahayaan untuk setiap sudut ruang.</p><a href="#collection" class="button">Jelajahi koleksi ↓</a><span class="hero-circle" aria-hidden="true"></span><div class="light-scene" aria-hidden="true"><span class="pendant-cord"></span><span class="pendant-shade"></span><span class="light-beam"></span><span class="light-pool"></span></div></section><aside class="light-dock" aria-label="Kontrol cahaya halaman"><label for="dimmer">☀ Cahaya <output id="dimmer-value" for="dimmer">70%</output></label><input id="dimmer" type="range" min="0" max="100" value="70" aria-label="Cahaya halaman" aria-valuetext="70 persen"></aside><script src="/experience.js" defer></script><section class="featured-story"><div class="featured-photo"><img src="/images/kastor-iris-reguler-matt-white.jpg" alt="Kastor Iris Reguler Matt-White" loading="lazy"></div><div class="featured-copy"><p class="eyebrow">IN THE SPOTLIGHT / IRIS SERIES</p><h2>Small details.<br>Beautiful light.</h2><p>Kenali Iris Reguler Matt-White. Lihat detail ukuran, cutout, dan finishing untuk kebutuhan ruang Anda.</p><a class="button" href="/p/kastor-iris-reguler-matt-white/">Explore Iris ↗</a></div></section><section id="collection"><div class="section-top"><h2>Our collection</h2><span class="small">${products.filter(p=>!p.demo).length} produk · ${products.filter(p=>p.demo).length} contoh demo</span></div><p class="notice">Produk berlabel DEMO menggunakan foto AI dan spesifikasi dummy.</p>${filters()}<div class="grid">${cards}</div></section>`));
